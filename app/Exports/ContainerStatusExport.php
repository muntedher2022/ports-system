...
class ContainerStatusExport implements FromArray, WithTitle, WithStyles, WithColumnWidths, WithEvents, WithStrictNullComparison
{
    ...
    {
        return $this->containerType === 'abandoned' ? 'الحاويات المتخلفة' : 'الحاويات الخطرة';
    }
    ...
    public function array(): array
    {
        ...
        // Entities
        $entities = ContainerEntity::active()
            ->orderBy('entity_type')
            ->orderBy('sort_order')
            ->get();
        ...
        $titleCols = array_merge(
            ["موقف الحاويات {$typeLabel} في {$portName} لغاية شهر {$monthName} لسنة {$yearName}"],
