<x-filament-widgets::widget>
    ...
    <div class="gcpi-risk-wrapper">
        <div class="gcpi-risk-header">
            <h3>
                <span>🛡️</span>
                <span>مركز الذكاء الرقابي ومؤشرات الامتثال التشغيلي (سنة {{ $data['currentYear'] }})</span>
            </h3>
            <div class="flex items-center gap-2">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                    ...
                    <span class="font-mono text-sm">{{ $data['healthScore'] }}%</span>
                </span>
            </div>
        </div>
        ...
        <div class="gcpi-risk-grid">
            ...
            <div class="gcpi-risk-panel panel-iso">
                <div>
                    ...
                    <div class="flex items-baseline justify-between mb-1">
                        <span class="text-2xl font-black text-blue-700 font-mono">{{ $data['isoStats']['compliance_rate'] }}%</span>
                        <span class="text-xs text-gray-500 font-semibold">{{ number_format($data['isoStats']['valid_count']) }} مطابقة من أصل {{ number_format($data['isoStats']['total_examined']) }}</span>
                    </div>

                    <div class="gcpi-progress-bar">
                        <div class="gcpi-progress-fill bg-blue-600" style="width: {{ $data['isoStats']['compliance_rate'] }}%"></div>
                    </div>
                </div>

                <div class="pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-semibold">
                    <span class="text-amber-700">⚠️ ملاحظات تدقيق / نواقص: {{ number_format($data['isoStats']['invalid_count']) }}</span>
                    <a href="{{ url('/admin/container-items') }}" class="text-blue-600 hover:text-blue-800 underline">فحص القيود ←</a>
                </div>
            </div>
            ...
            <div class="gcpi-risk-panel panel-danger">
                <div>
                    ...
                    <div class="flex items-baseline justify-between mb-1">
                        <span class="text-2xl font-black text-rose-700 font-mono">{{ number_format($data['dangerousCont'] + $data['dangerousCargo']) }}</span>
                        <span class="text-xs text-rose-600 font-semibold">رصيد خطر إجمالي</span>
                    </div>

                    <div class="gcpi-progress-bar">
                        <div class="gcpi-progress-fill bg-rose-500" style="width: {{ min(100, $data['contDangerRate'] * 3) }}%"></div>
                    </div>
                </div>

                <div class="pt-3 border-t border-rose-100 flex items-center justify-between text-xs font-semibold text-rose-800">
                    <span>حاويات خطرة: {{ number_format($data['dangerousCont']) }} ({{ $data['contDangerRate'] }}%)</span>
                    <span>طرود بضائع: {{ number_format($data['dangerousCargo']) }}</span>
                </div>
            </div>
            ...
            <div class="gcpi-risk-panel panel-aging">
                <div>
                    ...
                    <div class="flex items-baseline justify-between mb-1">
                        <span class="text-2xl font-black text-purple-700 font-mono">{{ number_format($data['oldContCount'] + $data['oldCargoCount']) }}</span>
                        <span class="text-xs text-purple-600 font-semibold">متروكة منذ سنوات سابقة</span>
                    </div>

                    <div class="gcpi-progress-bar">
                        <div class="gcpi-progress-fill bg-purple-600" style="width: {{ min(100, (($data['oldContCount'] + $data['oldCargoCount']) > 0 ? 65 : 10)) }}%"></div>
                    </div>
                </div>

                <div class="pt-3 border-t border-purple-100 flex items-center justify-between text-xs font-semibold text-purple-800">
                    <span>حاويات قديمة: {{ number_format($data['oldContCount']) }}</span>
                    <span>بضائع قديمة: {{ number_format($data['oldCargoCount']) }}</span>
                </div>
            </div>
        </div>
    </div>
