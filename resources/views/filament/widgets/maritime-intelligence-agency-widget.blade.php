        ...
        <div class="gcpi-agency-header">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[0.72rem] text-slate-400 font-medium">مؤشرات الكفاءة النوعية، التوقعات التراكمية، ومصفوفة قيادة وتصدر الموانئ لسنة {{ $m['currentYear'] }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 text-xs font-semibold bg-slate-800/90 px-3.5 py-1.5 rounded-xl border border-slate-700">
                <div class="text-right">
                    ...
                    <span class="text-amber-400 font-mono font-bold">{{ number_format($m['projectedAnnualTons']) }} طن</span>
                    <span class="text-slate-500 mx-1">|</span>
                    <span class="text-emerald-400 font-mono font-bold">{{ number_format($m['projectedAnnualRev'] / 1000000000, 2) }} مليار د.ع</span>
                </div>
            </div>
        </div>
            ...
            <div class="gcpi-agency-grid-kpi">
                ...
                <div class="gcpi-agency-card agency-card-cyan">
                    ...
                    <div class="text-lg font-black text-slate-900 font-mono mb-1">
                        {{ number_format($m['revPerTon']) }} <span class="text-xs font-medium text-slate-500">د.ع/طن</span>
                    </div>
                    <div class="text-[0.72rem] font-semibold flex items-center gap-1.5 {{ $m['revPerTonDiff'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">
                        <span>{{ $m['revPerTonDiff'] >= 0 ? '▲ +' : '▼ ' }}{{ $m['revPerTonDiff'] }}%</span>
                        <span class="text-slate-500 font-normal">مقارنة مع سنة {{ $m['prevYear'] }}</span>
                    </div>
                </div>
                ...
                <div class="gcpi-agency-card agency-card-emerald">
                    ...
                    <div class="text-lg font-black text-slate-900 font-mono mb-1">
                        {{ number_format($m['payloadPerShip']) }} <span class="text-xs font-medium text-slate-500">طن/سفينة</span>
                    </div>
                    <div class="text-[0.72rem] font-semibold flex items-center gap-1.5 {{ $m['payloadDiff'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">
                        <span>{{ $m['payloadDiff'] >= 0 ? '▲ +' : '▼ ' }}{{ $m['payloadDiff'] }}%</span>
                ...
                <div class="gcpi-agency-card agency-card-indigo">
                    ...
                    <div class="text-[0.72rem] text-slate-600 font-semibold flex items-center justify-between">
                        <span>📥 مستورد: {{ number_format($m['currImpTeu']) }}</span>
                        <span>📤 مصدر: {{ number_format($m['currExpTeu']) }}</span>
                    </div>
                </div>
                ...
                <div class="gcpi-agency-card agency-card-amber">
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        <span class="text-[0.68rem] px-2 py-0.5 rounded {{ $m['contRiskPercent'] > 10 ? 'bg-rose-100 text-rose-700 border-rose-300' : 'bg-amber-50 text-amber-700 border-amber-200' }} font-bold border">
                            {{ $m['contRiskPercent'] > 10 ? 'تنبيه رقابي ⚠️' : 'مستوى معتدل 🟢' }}
                        </span>
                    </div>
                    <div class="text-lg font-black text-slate-900 font-mono mb-1">
                        {{ $m['contRiskPercent'] }}% <span class="text-xs font-medium text-slate-500">من إجمالي الساحات</span>
                    </div>
                    <div class="text-[0.72rem] text-slate-600 font-semibold truncate">
                        <span>{{ number_format($m['dangerContDet']) }} حاوية خطرة من إجمالي {{ number_format($m['totalContDet']) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="gcpi-agency-dominance-row">
                <div class="gcpi-dom-box">
                    ...
                    <div class="truncate">
                        ...
                        <span class="text-xs font-black text-slate-800">{{ $m['topContainerHub']['name'] }}</span>
                        <span class="text-[0.7rem] text-blue-600 font-mono font-bold">({{ number_format($m['topContainerHub']['val']) }} TEU)</span>
                    </div>
                </div>
                ...
                <div class="gcpi-dom-box">
                    ...
                    <div class="truncate">
                        ...
                        <span class="text-xs font-black text-slate-800">{{ $m['topCargoHub']['name'] }}</span>
                        <span class="text-[0.7rem] text-amber-600 font-mono font-bold">({{ number_format($m['topCargoHub']['val']) }} طن)</span>
                    </div>
                </div>
                ...
                <div class="gcpi-dom-box">
                    ...
                    <div class="truncate">
                        ...
                        <span class="text-xs font-black text-slate-800">{{ $m['topOilHub']['name'] }}</span>
                        <span class="text-[0.7rem] text-purple-600 font-mono font-bold">({{ number_format($m['topOilHub']['val']) }} طن)</span>
                    </div>
                </div>
                ...
                <div class="gcpi-dom-box">
                    ...
                    <div class="truncate">
                        ...
                        <span class="text-xs font-black text-slate-800">{{ $m['topRevenueHub']['name'] }}</span>
                        <span class="text-[0.7rem] text-emerald-600 font-mono font-bold">({{ number_format($m['topRevenueHub']['val'] / 1000000000, 2) }} مليار د.ع)</span>
                    </div>
                </div>
            </div>
        </div>
